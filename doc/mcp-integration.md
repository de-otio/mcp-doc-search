...
```json
{
  "mcpServers": {
    "doc-search": {
      "command": "node",
      "args": ["~/.doc-search/bin/mcp-server.js"],
      "env": {
...
The `args` path is the **stable launcher** (written as an absolute path): a
tiny forwarder under `~/.doc-search/bin/` that the extension refreshes on
every activation to target the currently installed build. Configs pointing at
it keep working across extension upgrades. (Configs generated by older
versions embed the versioned extension path instead; the extension repairs
those to the stable path when the workspace is next opened.)
...
Prefer the stable launcher path (`~/.doc-search/bin/mcp-server.js`, written
out as an absolute path) — it survives extension upgrades. Pointing directly
at the extension install works too but embeds the version number, so it
